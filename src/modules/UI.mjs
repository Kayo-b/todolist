...
export default class DOM {
  ...
  static taskInput(projName) {
    ...
    taskInput.insertAdjacentHTML(
      "beforeend",
      `<div id="tasksInputField"> 
  ...
  static createProjObject() {
    ...
    else if (DOM.getIndexOfProject(projName) >= 0)
      return alert("project name already used");
  ...
  static getIndexOfProject(projID) {
    ...
    var indexOfTargetId = getTodoList.projects
      .map((x) => x.name)
      .indexOf(`${projID}`);
  ...
  static addTask(objId, taskName) {
    ...
    else if (DOM.getIndexOfTask(objId, taskName) >= 0)
      return alert("task already listed");
  ...
  static taskCheck() {
    ...
    checkbox.addEventListener("change", (e) => {
      ...
      if (
        projObj.tasks[indexOfTask].note != "" &&
        DOM.checkIfTaskIsInDue(e.target.id) == true
      ) {
        if (projObj.name == "Due / Overdue Tasks") {
          ...
          let todayTaskIndex = DOM.getIndexOfTask(
            "Due / Overdue Tasks",
            e.target.id
          );
  ...
  static assignMethodsToProjectObj(projectID) {
    ...
    var newObject = Object.assign(
      createProject(),
      getTodoList.projects[indexOfObject]
    );
  ...
  static deleteProjectButton() {
    ...
    deleteButtonTeste.addEventListener("click", function () {
      DOM.deleteProjectObj(siblingId2());
    });
  }
  ...
  static selectProjOnClick() {
    ...
    clickOnProject.addEventListener("click", (e) => {
      if (
        e.target.className != "projListingClass" &&
        e.target.className != "DueTasks"
      )
        return;
  ...
  static removeBottomBorder(clickOnProject) {
    let array = [];
    for (let x = 0; x < clickOnProject.children.length; x++)
      array.push(clickOnProject.children[x]);
    array.forEach(
      (item) => (item.style = "border-bottom: 2px solid transparent;")
    );
  }
